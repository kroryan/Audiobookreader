...
class MimiEncoderWrapper(nn.Module):
    ...
    def __init__(self, mimi, speaker_proj_weight, bos_before_voice=None):
        ...
        # Current language models require the learned fuser padding value in
        # front of the encoded voice prompt. The upstream PyTorch path adds it
        # in TTSModel.get_state_for_audio_prompt(); the C++ runtime consumes the
        # encoder output directly, so it must be part of this ONNX output.
        self.bos_before_voice = bos_before_voice
    ...
    def forward(self, audio: torch.Tensor) -> torch.Tensor:
        ...
        if self.bos_before_voice is not None:
            conditioning = torch.cat([self.bos_before_voice, conditioning], dim=1)
        return conditioning


def export_mimi_encoder(model: TTSModel, output_path: Path):
    bos_before_voice = (
        model.flow_lm.bos_before_voice
        if model.flow_lm.insert_bos_before_voice
        else None
    )
    wrapper = MimiEncoderWrapper(
        model.mimi,
...
def validate_mimi_encoder(model: TTSModel, onnx_dir: Path,
    ...
    with torch.no_grad():
        ...
        if model.flow_lm.insert_bos_before_voice:
            pt_conditioning = torch.cat([model.flow_lm.bos_before_voice, pt_conditioning], dim=1)
...
def validate_flow_lm_main(model: TTSModel, onnx_dir: Path,
    ...
    for mn, ms in pt_state.items():
        if "layers.0.self_attn" in mn:
            ...
            # ONNX deliberately stores the KV cache as FP16. A roughly 2e-3
            # absolute discrepancy is expected after that conversion.
            ok, msg = _compare("kv_cache_L0_K", pt_cache_k, ort_cache_k, atol=max(atol, 2e-3))
...
def main():
    ...
    parser.add_argument("--language", default="german",
                        help="Pocket-TTS language config to export (default: german)")
    parser.add_argument("--validate-only", action="store_true",
                        help="Skip export/quantize, only validate existing ONNX files")
    parser.add_argument("--no-validate", action="store_true",
                        help="Skip validation")
    parser.add_argument("--no-quantize", action="store_true",
                        help="Skip creation of INT8 model variants")
    ...
    # Load the requested upstream model. TTSModel resolves and caches the
    # official voice-cloning weights, including the normal German model.
    ...
    print("Loading model...")
    model = TTSModel.load_model(language=None if args.config else args.language,
                                config=str(config_path) if args.config else None)
    model.eval()
    print(f"  Voice cloning: {model.has_voice_cloning}")

    # The C++ runtime needs a standalone SentencePiece file. The upstream
    # config stores it as an hf:// URL; resolve that pinned URL directly.
    tokenizer_file = output_dir / "tokenizer.model"
    if not tokenizer_file.exists():
        tokenizer_url = str(config.flow_lm.lookup_table.tokenizer_path)
        if not tokenizer_url.startswith("hf://"):
            raise RuntimeError(f"Unsupported tokenizer URL: {tokenizer_url}")
        repo_and_path, revision = tokenizer_url[5:].rsplit("@", 1)
        repo, path = repo_and_path.split("/", 2)[:2], None
        # hf://owner/repo/path@revision
        parts = repo_and_path.split("/")
        url = f"https://huggingface.co/{parts[0]}/{parts[1]}/resolve/{revision}/{'/'.join(parts[2:])}"
        print("Downloading tokenizer...")
        _download(url, tokenizer_file)
