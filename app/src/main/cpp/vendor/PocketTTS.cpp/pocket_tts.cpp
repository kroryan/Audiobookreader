...
static std::optional<Tensor> load_npy_float32(const std::string& path) {
    std::ifstream input(path, std::ios::binary);
    if (!input) return std::nullopt;
    std::vector<char> bytes(
        (std::istreambuf_iterator<char>(input)), std::istreambuf_iterator<char>());
    if (bytes.size() < 10 || std::string(bytes.data(), 6) != "\x93NUMPY") return std::nullopt;
    const unsigned major = static_cast<unsigned char>(bytes[6]);
    size_t headerLength = 0;
    size_t dataOffset = 0;
    if (major == 1) {
        headerLength = static_cast<unsigned char>(bytes[8]) |
    ...
    if (dataOffset + headerLength > bytes.size()) return std::nullopt;
    const std::string header(bytes.data() + dataOffset, headerLength);
    if (header.find("'descr': '<f4'") == std::string::npos &&
        header.find("\"descr\": \"<f4\"") == std::string::npos) return std::nullopt;
    const size_t shapeStart = header.find("shape");
    if (shapeStart == std::string::npos) return std::nullopt;
    const size_t open = header.find('(', shapeStart);
    ...
    std::string part;
    while (std::getline(shapeStream, part, ',')) {
        std::stringstream valueStream(part);
        int64_t value = 0;
        if (valueStream >> value && value > 0) shape.push_back(value);
    }
    if (shape.empty()) return std::nullopt;
    const size_t count = calc_numel(shape);
    const size_t payloadOffset = dataOffset + headerLength;
    if (payloadOffset + count * sizeof(float) > bytes.size()) return std::nullopt;
    std::vector<float> values(count);
    std::memcpy(values.data(), bytes.data() + payloadOffset, count * sizeof(float));
    return Tensor(std::move(values), std::move(shape));
}
...
public:
    ...
    explicit PocketTTS(const Config& cfg = {}) : cfg_(cfg) {
        ...
        bos_before_voice_ = load_npy_float32(cfg_.models_dir + "/bos_before_voice.npy");
    ...
    Tensor encode_voice(const std::string& path) {
        ...
        if (bos_before_voice_ && bos_before_voice_->shape.size() == 3 &&
            bos_before_voice_->shape[0] == r.shape[0] &&
            bos_before_voice_->shape[2] == r.shape[2]) {
            r = Tensor::concat({*bos_before_voice_, r}, 1);
        }
...
private:
    ...
    std::optional<Tensor> bos_before_voice_;
    ...
    static uint64_t voice_hash(const Tensor& v) {
        ...
        // The first conditioning frame is the same BOS embedding for every
        // voice. Hashing only its first values therefore made all voices in a
        // model share the first voice's in-memory KV snapshot until restart.
        // Hash the complete speaker conditioning so a voice switch reliably
        // selects the matching snapshot while preserving the fast disk cache.
...
int ptt_stream_read(void* stream_ctx, float** out_samples, int* out_len) {
    ...
    ctx->cv.wait(lock, [ctx]{ return !ctx->chunks.empty() || ctx->done || ctx->aborted; });
...
// call ptt_stream_end() after its blocking read returns.
